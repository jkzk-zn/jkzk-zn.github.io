(() => {
  ...
  const TARGET_BY_MODE = Object.fromEntries(TARGETS.map((t) => [t.mode, t]));
  const PRIORITY_TARGETS = ["mode3", "mode2", "mode1"]
    .map((m) => TARGET_BY_MODE[m])
    .filter(Boolean);
  const INTERNAL_PRIORITY_TARGETS = ["mode3", "mode2"]
    .map((m) => TARGET_BY_MODE[m])
    .filter(Boolean);
  ...
  const InternalWatch = (() => {
    ...
    async function runOnce() {
      ...
      running = true;
      try {
        const externalTarget = TARGET_BY_MODE.mode1;
    ...
    function enable(intervalMs = 1500) {
      clear();
      if (!INTERNAL_TARGETS.length && !TARGET_BY_MODE.mode1) return;
